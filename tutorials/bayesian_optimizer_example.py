...
def parse_extended_arguments(comm):
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            "bukin", "eggcrate", "himmelblau", "keane", "leon", "rastrigin",
            "schwefel", "sphere", "step", "rosenbrock", "quartic", "bisphere",
            "birastrigin", "griewank", "all",
        ],
        default="sphere",
    ...
    parser.add_argument("--bo-initial-design", type=str, choices=["sobol", "random", "lhs"], default="sobol", help="Initial design strategy")
    ...
    parser.add_argument("--bo-p-explore-tau", type=float, default=150.0, help="Decay constant (higher = slower decay) for epsilon-greedy exploration")
    parser.add_argument("--bo-no-anneal", action="store_true", help="Disable automatic annealing of acquisition parameter (xi/kappa)")
    # Dynamic acquisition switching
    parser.add_argument("--bo-second-acq", type=str, choices=["EI", "PI", "UCB"], default=None, help="Optional second acquisition type to switch to")
    ...
    parser.add_argument("--bo-preset", type=str, choices=["balanced", "explore", "exploit", "aggressive"], default=None, help="Preset configuration overriding individual BO flags unless explicitly set")
    ...
    if config.bo_preset:
        ...
        if preset == "balanced":
            ...
            if config.bo_n_candidates is None: config.bo_n_candidates = 1024
            if config.bo_n_restarts is None: config.bo_n_restarts = 15
        elif preset == "explore":
            config.bo_acq = "UCB"
            config.bo_kappa = 3.5 if parser.get_default("bo_kappa") == config.bo_kappa else config.bo_kappa
            if config.bo_n_candidates is None: config.bo_n_candidates = 2048
            if config.bo_n_restarts is None: config.bo_n_restarts = 20
            config.bo_p_explore_start = 0.3
            config.bo_p_explore_end = 0.02
        elif preset == "exploit":
            config.bo_acq = "EI"
            config.bo_xi = 0.02
            if config.bo_n_candidates is None: config.bo_n_candidates = 512
            if config.bo_n_restarts is None: config.bo_n_restarts = 10
            config.bo_p_explore_start = 0.15
            config.bo_p_explore_end = 0.01
        elif preset == "aggressive":
            config.bo_acq = "EI"
            config.bo_xi = 0.1
            if config.bo_n_candidates is None: config.bo_n_candidates = 4096
            if config.bo_n_restarts is None: config.bo_n_restarts = 25
